...
#ifdef TRANSPORT_CTRLS_ABOVE_TRACK_HEADER
...
#ifndef TRANSPORT_CTRLS_ABOVE_TRACK_HEADER
...
RecorderUI::InputPort::InputPort (string const& name, DataType dt, RecorderUI* parent, bool vertical)
	...
	, _monitor_button (_("Mon"), ArdourButton::default_elements)
	, _name_button (name)
...
RecorderUI::InputPort::monitor_press (GdkEventButton* ev)
{
	...
	if (Keyboard::modifier_state_equals (ev->state, Keyboard::ModifierMask (Keyboard::PrimaryModifier|Keyboard::TertiaryModifier))) {
		...
		_solo_release = 0;
		return false;
	} else if (Keyboard::modifier_state_contains (ev->state, Keyboard::ModifierMask (Keyboard::PrimaryModifier|Keyboard::SecondaryModifier)) || Config->get_exclusive_solo ()) {
		/* Primary-Secondary-click: exclusive solo */
		if (_solo_release) {
			_solo_release->set_exclusive ();
			s->prepare_exclusive_solo (_solo_release);
		} else {
			/* clear solo state */
			s->prepare_exclusive_solo ();
		}
	} else {
		if (_solo_release) {
			boost::shared_ptr<std::list<std::string> > pml (new std::list<std::string>);
			mp.active_monitors (*pml);
			_solo_release->set (pml);
		}
	}
